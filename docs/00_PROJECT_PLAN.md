...
- [x] **0.24** Rewrite `server/*.test.ts` against Supabase hooks / server functions (or delete if obsolete). (done: 2026-05-09 — all 5 legacy `server/*.test.ts` files imported deleted tRPC modules (`./routers`, `../shared/const`) and were obsolete. Deleted them, repointed `vitest.config.ts` at `src/**/*.test.ts(x)` with the `@` → `src/` alias only, and added `src/lib/csv-export.test.ts` (4 tests, all passing) covering UTF-8 BOM, quote/comma/newline escaping, formula-injection mitigation, and null cells.)
...
- [x] **1.6** Add a `sql/_check.sh` helper for CI that runs all migrations against a throwaway DB. (done: 2026-05-09 — bash script spins up `postgres:15` in docker on port 55432 (or honours `PGURL`), runs every `NNNN_*.sql` in order via `psql -v ON_ERROR_STOP=1`, and runs the full set TWICE to assert idempotency. Skips `*.legacy.bak` / `*.mysql.bak`. Cleans up the container on exit.)
...
- [x] **5.4** Password reset, email verification, account deletion. (done: 2026-05-09 — password reset DONE via `/reset-password` route + `resetPasswordForEmail` on `/admin/login`. Email verification DONE via Supabase's default confirm-email flow. **Account deletion DONE** via new `supabase/functions/admin-delete-user/index.ts`: verifies caller's JWT (`verify_jwt = true`, no config.toml override needed), allows self-delete unconditionally, requires `admin`/`super_admin` role for cross-user delete, refuses to delete the last remaining admin (returns 409), then calls `auth.admin.deleteUser()` which cascades through `admin_profiles` + `admin_roles` via the existing FKs in `0001_init.sql`. Hook `useDeleteAdminUser` in `src/lib/supabase-hooks.ts`. UI on `/admin/users`: per-row "Delete" button (labelled "Delete me" for the current user), confirmation dialog requires typing `DELETE`, on self-delete signs out and redirects to `/admin/login`. **Action required:** `supabase functions deploy admin-delete-user --project-ref gnlvmszcysogydomahbk`.)
...
- [x] **6.3** WhatsApp Business Cloud API auto-reply (Meta). `sql/0014_whatsapp_send_log.sql` + `supabase/functions/send-whatsapp` + `src/lib/whatsapp.ts`. Mirrors the SMS pipeline (recipient locked to DB row, 5-min idempotency, four templates: reservation/order/status/payment). Owner must approve message templates in Meta Business Manager and set `WHATSAPP_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_TEMPLATE_RESERVATION/ORDER/STATUS/PAYMENT`, optional `WHATSAPP_TEMPLATE_LANG` (default `en`); function returns `not_configured` (silent skip) until set.
...
- [x] **7.4b** Add deposit payment on Reservations form. (done: 2026-05-09 — `sql/0012_reservation_deposits.sql` makes `payments.order_id` nullable, adds `payments.reservation_id` (FK + index) with a XOR check constraint so each payment row targets exactly one of order/reservation; adds `reservation_leads.deposit_amount` (int>0, nullable) + `reservation_leads.deposit_status` text default `'unpaid'` (CHECK in `unpaid|paid|waived`) with index; seeds `site_settings.reservation_deposit_amount = '0'`. `mpesa-initiate` now accepts `{ orderId? | reservationId? }` (XOR), validates the requested amount against either `orders.total_amount` or `reservation_leads.deposit_amount`, and persists the right FK on the `payments` row. `mpesa-callback` branches on `payment.order_id` vs `payment.reservation_id`: orders flow unchanged (sets `payment_status='paid'` + fires `order_payment_receipt`); reservations flip `deposit_status='paid'` and auto-promote `status: pending → confirmed`. `src/lib/payments.ts` `InitiatePaymentInput` widened. `ReservationsPage` reads `reservation_deposit_amount` from settings and, when > 0, persists `deposit_amount` on the new lead, shows a "Pay deposit" CTA on the success card with STK Push polling modal + resend, and surfaces `Deposit Paid` confirmation. **Action required:** apply `sql/0012_reservation_deposits.sql` on Supabase, redeploy `mpesa-initiate` + `mpesa-callback`, set `site_settings.reservation_deposit_amount` to the desired amount in Admin → Settings.)
- [x] **7.5** Automated refund flow in Admin Orders (Daraja Reversal API). `sql/0013_payment_refunds.sql` adds refund columns; `mpesa-reversal` (admin-only) initiates the reversal; `mpesa-reversal-result` (public, verify_jwt=false) finalises status. Owner must set `MPESA_INITIATOR_NAME`, `MPESA_SECURITY_CREDENTIAL` (RSA-encrypted), `MPESA_REVERSAL_RESULT_URL`, optional `MPESA_REVERSAL_TOKEN` and deploy both new functions.
...
- [x] **8.5** WCAG 2.1 AA audit. (done: 2026-05-09 — built on the 2026-05-08 pass: skip-link in `PublicLayout`, `aria-label="Open navigation menu"` on mobile trigger, `id="main-content"`/`tabIndex={-1}` on `<main>`. New: `DialogContent` accepts an optional `description` prop that auto-renders an `sr-only` `DialogDescription`, silencing Radix's a11y warning across all callers; introduced `LoadingSpinner` (`role="status"` + `aria-live="polite"` + sr-only label); upgraded loading regions on `MenuPage` and `OrderTrackingPage` with `role="status"`/`aria-live`/`aria-busy`/`aria-hidden` on decorative skeletons, error card with `role="alert"`. Form inputs continue to inherit `aria-describedby`/`aria-invalid` from shadcn `Form`. Color contrast verified against the oklch token set in `src/index.css`.)
...
- [x] **9.3** Uptime monitoring (UptimeRobot) on `/`, `/menu`, and the health probe at `https://<project>.supabase.co/functions/v1/health`. Probe lives at `supabase/functions/health/index.ts`, returns `{status, db, version}` (200 ok / 503 degraded), 4s upstream timeout, public (verify_jwt=false). Owner step: deploy the function and add 3 HTTPS monitors at 5-min interval with alerts wired to ops.
- [ ] **9.4** Daily DB backup verification (Supabase backups + manual test restore quarterly).
- [x] **9.5** Runbook in `docs/RUNBOOK.md` — deploy, rollback, incident response. (done: 2026-05-09 — covers envs/URLs, deploy (Vercel + migrations + edge functions), rollback procedures for each layer, triage flow + symptom→fix table, backup/restore drill, routine ops cadence, useful commands, and escalation contacts.)
