...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: "Today's Reservations", value: stats?.reservationsToday ?? 0, sub: `${stats?.pendingReservations ?? 0} pending`, icon: CalendarCheck, to: "/admin/reservations", color: "text-blue-400" },
    { label: "Unread Messages", value: stats?.newMessages ?? 0, sub: "contact inquiries", icon: MessageSquare, to: "/admin/messages", color: "text-yellow-400" },
    { label: "Featured Menu Items", value: stats?.featuredMenuItems ?? 0, sub: "on menu", icon: UtensilsCrossed, to: "/admin/menu", color: "text-green-400" },
    { label: "Gallery Images", value: stats?.galleryCount ?? 0, sub: "uploaded images", icon: Images, to: "/admin/gallery", color: "text-purple-400" },
    { label: "Active Events", value: stats?.activeEvents ?? 0, sub: "live events", icon: PartyPopper, to: "/admin/events", color: "text-primary" },
    { label: "Pending Reservations", value: stats?.pendingReservations ?? 0, sub: "need action", icon: TrendingUp, to: "/admin/reservations", color: "text-orange-400" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/reservations" className="text-primary text-xs hover:underline">View all</Link>
          </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            { label: "Upload Gallery Photo", to: "/admin/gallery" },
            { label: "Create Event", to: "/admin/events" },
            { label: "View Messages", to: "/admin/messages" },
            { label: "Update Settings", to: "/admin/settings" },
